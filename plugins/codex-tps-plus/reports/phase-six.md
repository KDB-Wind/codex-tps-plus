...
- summary 临时文件原子替换、事件数/总字节数/文件数有界，产物没有 delta 正文或内容
  摘要字段；
- E2 参考复算使用记录内 `capturedAt`，并覆盖重复 token 快照、工具调用和多请求区间；
- loopback 端点校验、同步 JSON-RPC 响应、正常 runner 发送序列和 server request 路由。

本次自动化命令结果：`npm test` 76/76 通过；phase-six 专项测试 24/24 通过；
...
node tools/observe-probe.mjs --endpoint $endpoint --out $run --thread-id $threadId --duration-ms 60000
...
- 未测试 `schemaVersion` 默认 capture-only；已知字段类型错误只关闭受影响指标。
